...
describe("heartbeat comment wake batching", () => {
  ...
  it("promotes deferred comment wakes with their comments after the active run is cancelled", async () => {
    ...
    try {
      ...
      expect(String(promotedPayload.message ?? "")).toContain(`\"commentIds\":[\"${queuedComment.id}\"]`);
      expect(String(promotedPayload.message ?? "")).toContain(`\"latestCommentId\":\"${queuedComment.id}\"`);
      expect(String(promotedPayload.message ?? "")).toContain("\"requestedCount\":1");
      expect(String(promotedPayload.message ?? "")).toContain("\"includedCount\":1");
      expect(String(promotedPayload.message ?? "")).toContain("\"missingCount\":0");
