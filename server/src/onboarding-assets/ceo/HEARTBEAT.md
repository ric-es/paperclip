...
- Create subtasks with `POST /api/companies/{companyId}/issues`. Always set `parentId` and `goalId`. For non-child same-code-change follow-ups, reviews, QA, or release tasks, set `inheritExecutionWorkspaceFromIssueId` to the source issue and include `executionProvenance: { "handoffRole": "follow_up" | "review" | "qa" | "release" }`.
