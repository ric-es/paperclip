...
Shell safety: never pipe `curl`/`wget` output directly into `python`, `python3`, `bash`, `sh`, `node`, or any other interpreter. If you need to inspect JSON from the API, prefer `jq` when available, or save the response to a temporary file before reading it.
